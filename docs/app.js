...
function extractShapes(root) {
    ...
    const walk = (element, matrix) => {
        ...
        for (const child of Array.from(element.children))
            walk(child, ownMatrix);
    };
    for (const child of Array.from(root.children))
        walk(child, [1, 0, 0, 1, 0, 0]);
    return shapes;
}
function elementToShape(element, matrix, id) {
    const tag = localName(element);
...
function tableFromGroup(group, matrix, id) {
...
function markRelationConnectors(shapes) {
    const boxes = shapes.filter((shape) => ["rect", "ellipse", "text"].includes(shape.kind));
...
function paint(element, name, fallback) {
    const value = element.getAttribute(name);
    if (!value || value === "none")
        return value === "none" ? null : fallback;
    return value;
}
