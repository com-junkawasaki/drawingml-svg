...
- `web/app.ts` builds SVGraph, browser-local coverage diagnostics, an SVGraph sidecar JSON, an SVGraph presentation package projection, canonical SVG source download, DrawingML fragment XML, DrawingML-to-SVG import for basic shape, grouped shape, connector, and native table fragments, PresentationML slide XML, and a `.pptx` ZIP without Python or server APIs.
