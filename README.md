...
- Analysis: unsupported visual/layout attributes such as filters, masks, blend/isolation, fill/clip rules, gradient units/transforms/spread methods, paint order, patterns, rendering hints, vector effects, word spacing, dash offsets, multi-value text rotation, text/path length adjustments, unsupported markers, and unresolved paint servers are reported
